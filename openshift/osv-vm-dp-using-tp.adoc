...
[role="lead"]
Schützen Sie VMs in OpenShift Virtualization mithilfe von Snapshots und Backups. Dieses Verfahren umfasst die Erstellung eines AppVault mithilfe von ONTAP S3 Objektspeicher, die Konfiguration von Trident Protect zur Erfassung von VM-Daten, einschließlich Kubernetes-Ressourcenobjekten, persistenten Volumes und internen Images, sowie die Wiederherstellung der Daten bei Bedarf.
...
Die persistenten Datenträger der OpenShift Virtualisierungs-VMs können durch ONTAP-Speicher, der in den OpenShift Cluster integriert ist, unterstützt werden link:https://docs.netapp.com/us-en/trident/["Trident CSI"]. In diesem Abschnitt verwenden wir link:https://docs.netapp.com/us-en/trident/trident-protect/learn-about-trident-protect.html["Trident Protect"], um Snapshots und Backups von VMs einschließlich ihrer Datenvolumes im ONTAP Objektspeicher zu erstellen.

Bei Bedarf stellen wir dann eine Wiederherstellung aus einem Snapshot oder einem Backup her.

Trident Protect ermöglicht Snapshots, Backups, Wiederherstellungen und die Notfallwiederherstellung von Anwendungen und VMs auf einem OpenShift Cluster. Für OpenShift Virtualization VMs können mit Trident Protect Kubernetes-Ressourcenobjekte, persistente Volumes und interne Images geschützt werden.
