...
**NetApp Trident Protect**, als kostenloses Add-on zu NetApp Trident, bietet erweiterte Funktionen für das Anwendungsdatenmanagement, die die Funktionalität und Verfügbarkeit zustandsbehafteter Kubernetes-Anwendungen verbessern, die von NetApp ONTAP-Speichersystemen unterstützt werden. Trident Protect vereinfacht die Verwaltung, den Schutz und die Bewegung containerisierter Workloads sowohl in öffentlichen Clouds als auch in On-Premises-Umgebungen. Es bietet außerdem robuste Automatisierungsfunktionen über seine API und CLI.
